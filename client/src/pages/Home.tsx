  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <span
        ...
        style={{ color: dim ? "var(--hi-muted,#5c5c58)" : "var(--hi-text,#0a0a0a)" }}
      >
        <TeamLogo team={team} size={18} />
        {team}
      </span>
      <span
        className="hi-stat text-sm"
        style={{ color: leading ? "var(--hi-text,#0a0a0a)" : "var(--hi-muted,#5c5c58)" }}
      >
        {fmtLiveScore(score)}
      </span>
    </div>
    ...
    >
      ...
      <div className="flex items-center gap-1.5 pt-0.5" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <span
          ...
          style={{ color: live ? "#10B981" : pre ? "var(--hi-muted,#5c5c58)" : "var(--hi-muted,#5c5c58)" }}
        >
          {final ? "FINAL" : g.statusDetail || (pre ? "Scheduled" : "")}
        </span>
        {pre && g.tv && g.tv !== "Local" ? (
          <span className="text-[10px]" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>· {g.tv}</span>
        ) : null}
      </div>
    </div>
  );
        ...
        <div className="container">
          <div className="flex items-center gap-3 py-2 overflow-x-auto">
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className={`section-label text-xs ${anyLive ? "text-emerald-400" : ""}`} style={anyLive ? undefined : { color: "var(--hi-accent-text,#146a8c)" }}>
                {anyLive ? "LIVE" : "TODAY"}
              </span>
              {data?.fetchedAt ? (
                <span className="text-[10px] mono-data hidden sm:inline" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
...
function TickerBar() {
  ...
  return (
    <div
      ...
      style={{ borderColor: "var(--hi-muted,#5c5c58)", background: "rgba(0,0,0,0.4)" }}
    ...
    >
      ...
      <div className="flex items-center">
        ...
        <button
          ...
          style={{ background: "rgba(0,0,0,0.45)", color: "var(--hi-muted,#5c5c58)" }}
...
function HeroLeadStory() {
  ...
  return (
    ...
    >
      <span className="section-label text-[10px]" style={{ color: "var(--hi-accent-text,#146a8c)" }}>
        Tonight’s Marquee
      </span>
      ...
      <span className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>@</span>
      <span className="flex items-center gap-1.5 text-sm font-bold text-white">
      ...
      <div className="container py-12 md:py-16">
        <div className="max-w-4xl animate-fade-up">
          ...
          <div className="flex items-center gap-2 mb-6">
            <span className="text-xs font-semibold tracking-wide" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>BY</span>
            ...
            <span className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>Hoops Intel</span>
            <span style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>·</span>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {showMyPulse ? (
              ...
              <a
                ...
                style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-text,#0a0a0a)", border: "1px solid rgba(255,255,255,0.12)" }}
              >
                Set My Pulse
              </a>
            )}
          ...
          <details className="mt-4 max-w-xl rounded-lg border border-white/10 bg-black/25 open:bg-black/35 px-4 py-2">
            <summary className="text-xs cursor-pointer select-none outline-none hover:text-[var(--hi-text)] [&::-webkit-details-marker]:hidden [&::marker]:content-none flex items-center gap-2 justify-between py-2" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3 flex-1">
            <a href={`/team/${game.awayTeam.toLowerCase()}`} className="flex flex-col items-center w-14" onClick={(e) => e.stopPropagation()}>
              ...
              <div className={`hi-stat text-2xl ${awayWin ? "pulse-glow-blue" : ""}`} style={{ color: awayWin ? "var(--hi-text,#0a0a0a)" : "var(--hi-muted,#5c5c58)" }}>{game.awayScore}</div>
            </a>
            <div className="text-xs text-center" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>@</div>
            <a href={`/team/${game.homeTeam.toLowerCase()}`} className="flex flex-col items-center w-14" onClick={(e) => e.stopPropagation()}>
              ...
              <div className={`hi-stat text-2xl ${homeWin ? "pulse-glow-blue" : ""}`} style={{ color: homeWin ? "var(--hi-text,#0a0a0a)" : "var(--hi-muted,#5c5c58)" }}>{game.homeScore}</div>
            </a>
          </div>
          <div className="text-right">
            <div className="text-xs font-medium px-2 py-0.5 rounded mb-1 inline-block" style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-text,#0a0a0a)" }}>FINAL</div>
            <div className="text-xs" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{expanded ? "▲ Collapse" : "▼ Expand"}</div>
          </div>
        </div>
    ...
    <section id="scores" className="py-10">
      <div className="container">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div className="mono-data text-sm px-3 py-1 rounded" style={{ background: "rgba(142,200,240,0.1)", color: "var(--hi-accent-text,#146a8c)", border: "1px solid rgba(142,200,240,0.2)" }}>
        ...
        {sortedGames.length > 0 ? (
          ...
          >
            <p className="section-label mb-2" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>NO GAMES</p>
...
function CollapsibleEditionExtras({
  ...
  return (
    <section className="py-6 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
        <button
          type="button"
...
function NarrativeSection() {
  return (
    <section className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
              ...
              {statLeaders.map((stat: any, i: number) => (
                <div key={i} className="glass-card rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs font-medium mb-1" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{stat.category.toUpperCase()}</div>
                    ...
                    <div className="text-right">
                      <div className="mono-data text-2xl font-bold" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{stat.value}</div>
                    </div>
                  </div>
                </div>
              ))}
...
function PulseNoteModal({
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
          <div>
...
function PulseIndexSection() {
  ...
  return (
    <section id="pulse-index" className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
        <div className="flex items-center justify-between mb-2">
          <div className="section-label">{finalsOn ? "FINALS PULSE" : "DAILY RANKINGS"}</div>
          <a href="/pulse-history" className="text-xs font-medium hover:underline" style={{ color: "var(--hi-accent-text,#146a8c)" }}>View History &rarr;</a>
        </div>
        ...
        {finalsOn && pulseRows.length === 0 ? (
          <p className="text-sm rounded-lg px-4 py-3 mb-4" style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-muted,#5c5c58)" }}>
          ...
          {pulseRows.map((player: any) => {
            ...
            return (
              <div key={player.rank} className="glass-card rounded-lg p-4 flex items-center gap-4" style={isFavorite ? { borderLeft: "3px solid var(--hi-accent)" } : {}}>
                <div className="mono-data text-2xl font-bold w-8 text-center" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{player.rank}</div>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <a href={`/team/${player.team.toLowerCase()}`} className="inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded hover:bg-white/10 transition-colors" style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-muted,#5c5c58)" }}><TeamLogo team={player.team} size={14} />{player.team}</a>
                    {isFavorite && (
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="text-right">
                    <div className="mono-data text-lg font-bold" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{player.indexScore}</div>
                    <div className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{player.teamRecord}</div>
                  </div>
...
function MediaReactionsSection() {
  const sentimentColors: Record<string, { color: string; bg: string }> = {
    ...
    cold: { color: "var(--hi-accent-text,#146a8c)", bg: "rgba(142,200,240,0.1)" },
  ...
  return (
    <section className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
        ...
        <p className="text-xs mb-6" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
            ...
            return (
              <div key={i} className="glass-card rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{reaction.topic}</span>
                </div>
                <p className="text-sm leading-relaxed mb-3" style={{ color: "var(--hi-muted,#5c5c58)" }}>{reaction.quote}</p>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-white">{reaction.author}</span>
                  <span className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{reaction.outlet}</span>
                </div>
              </div>
            );
          })}
...
function InjurySection() {
  ...
  return (
    <section id="injuries" className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
              ...
              <div key={i} className="glass-card rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <a href={`/team/${String(injury.team || "nba").toLowerCase()}`} className="text-xs px-1.5 py-0.5 rounded hover:bg-white/10 transition-colors shrink-0" style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-muted,#5c5c58)" }}>{injury.team || "NBA"}</a>
                  </div>
...
function PlayoffSection() {
  ...
  const statTiles = [
    ...
    { label: "Finals played", value: String(snap.gamesPlayed), color: "var(--hi-accent-text,#146a8c)" },
    {
  ...
  return (
    <section id="playoffs" className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
        <div className="flex items-center justify-between mb-2">
          <div className="section-label">POSTSEASON</div>
          <a href="/playoffs" className="text-xs font-medium" style={{ color: "var(--hi-accent-text,#146a8c)" }}>Full bracket &rarr;</a>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-6">
          {statTiles.map((t) => (
            <div key={t.label} className="glass-card rounded-lg p-2.5 text-center">
              ...
              <div className="text-[10px] mt-0.5" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
                {t.label}
              </div>
            </div>
          ))}
...
function HomeSeriesCard({ series }: { series: PlayoffSeries }) {
  ...
  return (
    <div className="glass-card rounded-lg p-3" style={{ borderLeft: `3px solid ${accent}` }}>
      ...
      {edge && (
        <div className="mt-2 text-[10px] leading-snug mono-data" style={{ color: "var(--hi-muted,#5c5c58)" }}>
      ...
      {nextGame && (
        <div className="mt-1 text-[11px] mono-data" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        <span
          ...
          style={{
            ...
            color: leading ? "var(--hi-accent-text,#146a8c)" : "var(--hi-muted,#5c5c58)",
          }}
        >
          {seed}
        </span>
      ...
      <div className="flex items-center gap-1">
        ...
        <span className="mono-data text-xs ml-1 font-bold" style={{ color: leading ? "var(--hi-accent-text,#146a8c)" : "var(--hi-muted,#5c5c58)" }}>{wins}</span>
      </div>
    </div>
  );
}
...
function PlayoffMoversCard() {
  return (
    <div className="mt-6 glass-card rounded-lg p-4">
      <div className="section-label mb-3" style={{ color: "var(--hi-accent-text,#146a8c)" }}>PLAYOFF RISERS &amp; FALLERS</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {playoffMovers.map((m, i) => (
          <div key={i} className="flex items-start gap-3 p-2 rounded" style={{ background: "rgba(255,255,255,0.02)" }}>
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold text-white">{m.player} <span className="text-xs font-normal" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{m.team}</span></div>
...
function TonightSection() {
  return (
    <section id="tonight" className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
        ...
        {gamePreviews.length > 0 ? (
          ...
          <div
            ...
            style={{ border: "1px dashed rgba(255,255,255,0.12)" }}
          >
            <p className="section-label mb-2" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>NO GAMES TONIGHT</p>
            <p className="text-sm" style={{ color: "var(--hi-muted,#5c5c58)" }}>
  ...
  return (
    <div className={`glass-card rounded-lg overflow-hidden ${preview.featured ? "ring-1 ring-transparent" : ""}`}>
      <div className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <a href={`/team/${preview.awayTeam.toLowerCase()}`} className="flex flex-col items-center gap-1">
              ...
              <div className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{preview.awayRecord}</div>
            </a>
            ...
            <a href={`/team/${preview.homeTeam.toLowerCase()}`} className="flex flex-col items-center gap-1">
              ...
              <div className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{preview.homeRecord}</div>
            </a>
          </div>
          <div className="text-right">
          ...
          <div className="text-right ml-4">
            <div className="mono-data text-xs" style={{ color: "var(--hi-accent-text,#146a8c)" }} title="Vegas point spread — negative means favored">
              {closer}
            </div>
            {moveBadge ? (
            ...
            <div className="mono-data text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }} title="Over/Under — projected combined total points">Total {preview.overUnder}</div>
          </div>
          <button
            type="button"
        ...
        {preview.featured && (
          <div className="mt-2 text-xs font-semibold px-2 py-0.5 rounded inline-block" style={{ background: "rgba(142,200,240,0.15)", color: "var(--hi-accent-text,#146a8c)" }}>
            FEATURED GAME
          </div>
        )}
        {series && (
        ...
        <div className="px-4 pb-4 space-y-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          {intel && (
            <div className="pt-3 space-y-2 mb-1" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: "0.75rem" }}>
              ...
              <p className="text-sm leading-relaxed" style={{ color: "var(--hi-muted,#5c5c58)" }}>{intel.narrative}</p>
            </div>
          )}
          <div className="pt-3">
            <div className="text-xs font-medium mb-1" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>KEY MATCHUP</div>
            <div className="text-sm text-white">{preview.keyMatchup}</div>
          </div>
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>STORYLINE</div>
            <p className="text-sm leading-relaxed" style={{ color: "var(--hi-muted,#5c5c58)" }}>{preview.storyline}</p>
          </div>
          <div>
            <div className="text-xs font-medium mb-1" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>PREDICTION</div>
            <p className="text-sm" style={{ color: "#10B981" }}>{preview.prediction}</p>
          </div>
          <div className="pt-2 space-y-1">
            <div className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>
            ...
            <div className="flex flex-wrap gap-2">
              <a
                ...
                style={{ background: "var(--hi-surface-2,#f3f3f0)", color: "var(--hi-muted,#5c5c58)", border: "1px solid rgba(255,255,255,0.1)" }}
              >
                DraftKings
              </a>
              <a
...
function RookieAndFantasySection() {
  return (
    <section id="fantasy" className="py-10 border-t" style={{ borderColor: "var(--hi-muted,#5c5c58)" }}>
      <div className="container">
            ...
            <div className="space-y-2">
              {rookieWatch.map((rookie: any) => (
                <div key={rookie.rank} className="glass-card rounded-lg p-3 flex items-center gap-3">
                  <div className="mono-data text-lg font-bold w-6 text-center" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{rookie.rank}</div>
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-white">
                      <a href={`/player/${slugify(rookie.player)}`} className="hover:text-[var(--hi-text)] transition-colors">{rookie.player}</a>{" "}
                      <a href={`/team/${rookie.team.toLowerCase()}`} className="text-xs hover:text-[var(--hi-text)] transition-colors" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{rookie.team}</a>
                    </div>
                ...
                return (
                  <div key={i} className="glass-card rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{actionLabels[alert.action]}</span>
                      ...
                      <a href={`/team/${alert.team.toLowerCase()}`} className="text-xs hover:text-[var(--hi-text)] transition-colors" style={{ color: "var(--hi-text-secondary,#5c5c58)" }}>{alert.team}</a>
                    </div>
    ...
    <div>
          ...
          <tbody>
            {sortedStandings(standings).map((team: any, idx: number) => (
              <tr
                ...
                className="border-t"
                style={{
                  borderColor: team.rank === 6 && sortKey === "rank" ? "rgba(142,200,240,0.4)" : "var(--hi-muted,#5c5c58)",
              ...
              >
                <td className="px-3 py-2 mono-data" style={{ color: "var(--hi-accent-text,#146a8c)" }}>{team.rank}</td>
      ...
      <div className="enhanced-card divide-y" style={{ borderColor: "var(--hi-border,#1e2c40)" }}>
        {sortedStandings(standings).map((team: any) => (
          ...
          >
            <span className="mono-data pulse-score w-6 text-sm" style={{ color: "var(--hi-accent-text,#146a8c)" }}>
              {team.rank}
            </span>
