...
|----------------|---------|------------|
| **`playoffSeries`** | Wins, schedules, finals, elimination flags per series; keyed by **`seriesId`** (`E|W|F` + round + teams). | **Automated:** `npm run playoff:sync` (ESPN scoreboard window → `.daily-data/playoff-series.json` → `sync-playoff-data.mjs` rewrites only the `BEGIN_PLAYOFF_SERIES_SYNC` … `END_PLAYOFF_SERIES_SYNC` region). **`generate-all-daily.mjs` runs the same preflight.** |
| **`playoffMovers`** | Risers/fallers: `delta`, **`playoffLine`** (composite stat string), **`note`**. | **Manual** (or future generator). Editorial; refresh after marquee games — keep lines consistent with Pulse edition tone. |
