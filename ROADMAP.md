...
Superseded by [`NEXT-STEPS.md`](./NEXT-STEPS.md) for near-term planning. This
document tracks what's shipped and what's still genuinely on the horizon.
...
- Live playoff series state (April 2026) — `PlayoffSeries` data model with
...
- Premium tier launch (Stripe subscription + pro-only features)
- Shipping the embeddable widget `<script>` bundle as an actual product
- Round 2+ playoff coverage refinements
